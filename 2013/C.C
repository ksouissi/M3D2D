...
#define SBLR    8   //10
#define PTS 6 * ((1 << SBLR) - 2) * ((1 << SBLR) - 2)   //172974
...
int rk[256];
int O[(1 + H * 8) * 2], A[520244 * 2], Q[/* 126553 */ 515169 /* 8351857 */ /* 33480825 */ * 2], PR[1396059 * 2];
...
int V[3] = { 128 << 8, 128 << 8, 128 << 8 }, D[3], B[3][3];
...
void M3D2D(int L0, int U0, int R0, int D0, int L1, int U1, int R1, int D1, int L2, int U2, int R2, int D2, int L3, int U3, int R3, int D3, int L4, int U4, int R4, int D4, int L5, int U5, int R5, int D5, int L6, int U6, int R6, int D6, int L7, int U7, int R7, int D7, int L8, int U8, int R8, int D8, int x0, int x1, int x2, int y0, int y1, int y2, unsigned O[], int t)
{
    int i, j, k;
...
#define V(i, j) (((128 << OH) - (V[0] & (256 << OH) - 1)) * B[0][j] + ((128 << OH) - (V[1] & (256 << OH) - 1)) * B[1][j] + ((128 << OH) - (V[2] & (256 << OH) - 1)) * B[2][j] + (((B[0][j] & i << 31 >> 31) << 1) - B[0][j] + ((B[1][j] & i << 30 >> 31) << 1) - B[1][j] + ((B[2][j] & i << 29 >> 31) << 1) - B[2][j] << OH + 7) >> 8)
#define K(i, j) (-(128 << OH) * B[0][j] - (128 << OH) * B[1][j] - (128 << OH) * B[2][j] + ((i << 31 >> 31 & B[0][j]) + (i << 30 >> 31 & B[1][j]) + (i << 29 >> 31 & B[2][j]) << OH << 8) >> 8)
...
//    CL(O, H);
    ...
    SB(P, SBLR);
    O3(Q, P, A, 6 * ((1 << SBLR) - 2) * ((1 << SBLR) - 2), SBLR, 0);
    CL(Q, SBLR);

    PY(P, 8);
    O3(PR, P, Q, 256 * 256, 8, 0);
    CL(PR, 8);
    ...
    for (;;) {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            for (i = !!GetAsyncKeyState(VK_UP) | !!GetAsyncKeyState(VK_DOWN) << 1 | !!GetAsyncKeyState(VK_LEFT) << 2 | !!GetAsyncKeyState(VK_RIGHT) << 3 /* A, START */, j = 3; j--; V[j] += (i << 31 >> 31 & B[j][2]) - (i << 30 >> 31 & B[j][2]) - (i << 29 >> 31 & B[j][0]) + (i << 28 >> 31 & B[j][0]));
            memset(W, 0xff, (XR * YR + 31) / 32 * 4);

            M3D2D(-V(0, 2), -V(0, 2) * YX, V(0, 2), V(0, 2) * YX, -V(1, 2), -V(1, 2) * YX, V(1, 2), V(1, 2) * YX, -V(2, 2), -V(2, 2) * YX, V(2, 2), V(2, 2) * YX, -V(3, 2), -V(3, 2) * YX, V(3, 2), V(3, 2) * YX, -V(4, 2), -V(4, 2) * YX, V(4, 2), V(4, 2) * YX, -V(5, 2), -V(5, 2) * YX, V(5, 2), V(5, 2) * YX, -V(6, 2), -V(6, 2) * YX, V(6, 2), V(6, 2) * YX, -V(7, 2), -V(7, 2) * YX, V(7, 2), V(7, 2) * YX, 0, 0, XR << Z, YR << Z, V[0] - (128 << OH), V[1] - (128 << OH), V[2] - (128 << OH), ((128 << OH) - V[0]) * B[0][0] + ((128 << OH) - V[1]) * B[1][0] + ((128 << OH) - V[2]) * B[2][0] >> 8, ((128 << OH) - V[0]) * B[0][1] + ((128 << OH) - V[1]) * B[1][1] + ((128 << OH) - V[2]) * B[2][1] >> 8, ((128 << OH) - V[0]) * B[0][2] + ((128 << OH) - V[1]) * B[1][2] + ((128 << OH) - V[2]) * B[2][2] >> 8, O, 0);
            if (ESB)
                if ((i = NU(0, 0, XR, YR)) >= 0)
                    M3D2D(-K(0, 2), -K(0, 2) * YX, K(0, 2), K(0, 2) * YX, -K(1, 2), -K(1, 2) * YX, K(1, 2), K(1, 2) * YX, -K(2, 2), -K(2, 2) * YX, K(2, 2), K(2, 2) * YX, -K(3, 2), -K(3, 2) * YX, K(3, 2), K(3, 2) * YX, -K(4, 2), -K(4, 2) * YX, K(4, 2), K(4, 2) * YX, -K(5, 2), -K(5, 2) * YX, K(5, 2), K(5, 2) * YX, -K(6, 2), -K(6, 2) * YX, K(6, 2), K(6, 2) * YX, -K(7, 2), -K(7, 2) * YX, K(7, 2), K(7, 2) * YX, 0, 0, XR << Z, YR << Z, 0, 0, 0, 0, 0, 0, A, i);
            BitBlt(hdc, 0, 0, XR, YR, hdcm, 0, 0, SRCCOPY);
