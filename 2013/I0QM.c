...
#define D4(x)   ((V_(x, 1) >= ((E[Z] >> 16) + (1 << (E[A] >> 8) >> 1)) * (V_(x, 2) >> LOG_R - 1) - V_(x, 2)) << 1 | (V_(x, 0) >= ((E[Z] & 0xffff) + (1 << (E[A] >> 8) >> 1)) * (V_(x, 2) >> LOG_R - 1) - V_(x, 2)))
...
L0:
            ...
            for (i = 7, j = 0; i >= 0; i--)
                j |= ((V_(i, 1) >= V_(i, 2)) << 3 | (V_(i, 0) >= V_(i, 2)) << 2 | (V_(i, 1) < -V_(i, 2)) << 1 | V_(i, 0) < -V_(i, 2)) << (i << 2);
            if (j) {
...
L1:
        if (O_)
L2:
            for (QD(E[Y], E[Y + 1], E[Y + 2], E[A] & 0xff, E[Z] & 0xffff, E[Z] >> 16, E[A] >> 8, E[Q], ((int *)E[O])[8] << 8 & 0xff0000 | ((int *)E[O])[8] >> 8 & 0xff00 | ((int *)E[O])[8] & 0xff);;) {
                E[Q] = QF(E[Q]);
                if (CNB(E[Q]))
                    break;
                B(E[Q]);
