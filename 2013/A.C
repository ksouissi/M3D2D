...
#define SBLR    7   //10
#define PTS 172974  //6 * ((1 << SBLR) - 2) * ((1 << SBLR) - 2)
...
void SB(int t[][4], int a)
{
    int x, y, z, i;

    for (a = (1 << a) - 1, z = (a - 1) * (a - 1), x = 1; x < a; x++)
        for (y = 1; y < a; y++)
            for (i = 3; i--; t[i * z + (x - 1) * (a - 1) + y - 1][i] = 0, t[(i + 3) * z + (x - 1) * (a - 1) + y - 1][i] = a << 8, t[i * z + (x - 1) * (a - 1) + y - 1][(i + 1) % 3] = t[(i + 3) * z + (x - 1) * (a - 1) + y - 1][(i + 1) % 3] = x << 8, t[i * z + (x - 1) * (a - 1) + y - 1][(i + 2) % 3] = t[(i + 3) * z + (x - 1) * (a - 1) + y - 1][(i + 2) % 3] = y << 8, t[i * z + (x - 1) * (a - 1) + y - 1][3] = t[(i + 3) * z + (x - 1) * (a - 1) + y - 1][3] = (x ^ y) << (i << 3));
}

#include <stdio.h>

void loadxyzrgba(char *xyz, int n, float s)
{
    int i, j;
    float k;
    FILE *f;
    ...
    if (f = fopen(xyz, "r")) {
        for (i = 0; i < n; i++) {
            for (j = 0; j < 3; fscanf(f, "%f", &k), t[i][j++] = k * s);
            for (; j >= 0; fscanf(f, "%d", (char *)&t[i][3] + 3 - j--));
        }
        fclose(f);
    }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    loadxyzrgba("armadillo", PTS, -(1732 >> H));
    for (C[i = 0] = C[1] = C[2] = (1 << 31) - 1; i < PTS; i++)
        for (j = 3; j--;)
            if (t[i][j] < C[j])
                C[j] = t[i][j];
    for (i = 0; i < PTS; i++)
    ...
    for (i = H; i > 0; i--)
        for (j = 0; j < 8; j++)
            O[(H - i) * 9 + j] = &O[(H - i + 1) * 9];
/*        O[(H - i) * 9 + 2] = O[(H - i) * 9 + 3] = O[(H - i) * 9 + 6] = O[(H - i) * 9 + 7] = &O[(H - i + 1) * 9]; */
    merge(t, PTS, &O[H * 9], O /* 0 */, OH / 2 - H + 8, 8);
    color(O /* &O[H * 9] */, OH / 2);

    SB(t, SBLR);
    sort(t, 0, 6 * ((1 << SBLR) - 2) * ((1 << SBLR) - 2) - 1);
    merge(t, 6 * ((1 << SBLR) - 2) * ((1 << SBLR) - 2), Q, 0, SBLR + 8, 8);
    color(Q, SBLR);
    ...
    for (;;) {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            M3D2D(-V(0, 2), -V(0, 2) * YX, V(0, 2), V(0, 2) * YX, -V(1, 2), -V(1, 2) * YX, V(1, 2), V(1, 2) * YX, -V(2, 2), -V(2, 2) * YX, V(2, 2), V(2, 2) * YX, -V(3, 2), -V(3, 2) * YX, V(3, 2), V(3, 2) * YX, -V(4, 2), -V(4, 2) * YX, V(4, 2), V(4, 2) * YX, -V(5, 2), -V(5, 2) * YX, V(5, 2), V(5, 2) * YX, -V(6, 2), -V(6, 2) * YX, V(6, 2), V(6, 2) * YX, -V(7, 2), -V(7, 2) * YX, V(7, 2), V(7, 2) * YX, 0, 0, XR << Z, YR << Z, V[0] - (128 << OH), V[1] - (128 << OH), V[2] - (128 << OH), V(0, 0), V(0, 1), V(0, 2), OH, O, 0);
            if (ESB) {
                if ((i = NU(0, 0, XR, YR)) >= 0)
