...
void D2D3M(int x0, int x1, int x2, int y0, int y1, int y2, int z, int O[], int x, int y, int t, unsigned Q)
{
    ...
    if (O_) {
        for (x0 = x * (y2 >> LOG_R - 1) - y2, x1 = y * (y2 >> LOG_R - 1) - y2, i = (y1 >= x1 + (y2 >> LOG_R - t)) << 1 | y0 >= x0 + (y2 >> LOG_R - t); t; x0 += i << 31 >> 31 & (y2 >> LOG_R - t), x1 += i << 30 >> 31 & (y2 >> LOG_R - t), --t, x += (i & 0x1) << t, y += i >> 1 << t, Q = QC(Q, i), i = (y1 >= x1 + (y2 >> LOG_R - t)) << 1 | y0 >= x0 + (y2 >> LOG_R - t))
            if (NW(QC(Q, i)))
                return;
        p[(y << LOG_R) + x] = O[8];
        do { B(Q); Q = QF(Q); } while (ACB(Q));
