...
#define ACB(x)  !(~quadtrees[QC(x, 0) >> 5] & 0xf << (QC(x, 0) & 0x1f))
...
#define V_0(i)   V0(i)
#define V_1(i)  (V1(i) - (B[0][i]                    ))
#define V_2(i)  (V2(i) - (          B[1][i]          ))
#define V_3(i)  (V3(i) - (B[0][i] + B[1][i]          ))
#define V_4(i)  (V4(i) - (                  + B[2][i]))
#define V_5(i)  (V5(i) - (B[0][i]           + B[2][i]))
#define V_6(i)  (V6(i) - (          B[1][i] + B[2][i]))
#define V_7(i)  (V7(i) - (B[0][i] + B[1][i] + B[2][i]))

#define DIR(i)  ((V_##i(1) >= M(U##i, D##i)) << 1 | V_##i(0) >= M(L##i, R##i))
...
void M3D2D(int x0, int x1, int x2, int y0, int y1, int y2, int z, int O[], int x, int y, int t, unsigned Q, int L0, int U0, int R0, int D0, int L1, int U1, int R1, int D1, int L2, int U2, int R2, int D2, int L3, int U3, int R3, int D3, int L4, int U4, int R4, int D4, int L5, int U5, int R5, int D5, int L6, int U6, int R6, int D6, int L7, int U7, int R7, int D7)
{
    ...
    p[(y - (1 << LOG_R + 1) << LOG_R) + x - (1 << LOG_R + 1)] = O[8];
    do { B(Q); Q = QF(Q); } while (ACB(Q));
}
#else
...
void M3D2D(int x0, int x1, int x2, int y0, int y1, int y2, int z, int O[], int x, int y, int t, unsigned Q, int L0, int U0, int R0, int D0, int L1, int U1, int R1, int D1, int L2, int U2, int R2, int D2, int L3, int U3, int R3, int D3, int L4, int U4, int R4, int D4, int L5, int U5, int R5, int D5, int L6, int U6, int R6, int D6, int L7, int U7, int R7, int D7)
{
    ...
    if (O_) {
        for (i = DIR(0); t; --t, x += (i & 0x1) << t, y += i >> 1 << t, L0 = L4(0, i), U0 = U4(0, i), R0 = R4(0, i), D0 = D4(0, i), Q = QC(Q, i), i = DIR(0))
            if (NW(QC(Q, i)))
                return;
        p[(y - (1 << LOG_R + 1) << LOG_R) + x - (1 << LOG_R + 1)] = O[8];
        do { B(Q); Q = QF(Q); } while (ACB(Q));
...
int mulsar2(int x, int y)
{
    __asm {
        mov     eax, x
        imul    y
        shrd    eax, edx, LOG_R - 1
    }
}
...
void D2D3M(int x0, int x1, int x2, int y0, int y1, int y2, int z, int O[], int x, int y, int t, unsigned Q)
{
    ...
    if (O_) {
        for (x0 = mulsar2(x, y2) - y2, x1 = mulsar2(y, y2) - y2, i = (y1 >= x1 + (y2 >> LOG_R - t)) << 1 | y0 >= x0 + (y2 >> LOG_R - t); t; x0 += i << 31 >> 31 & (y2 >> LOG_R - t), x1 += i << 30 >> 31 & (y2 >> LOG_R - t), --t, x += (i & 0x1) << t, y += i >> 1 << t, Q = QC(Q, i), i = (y1 >= x1 + (y2 >> LOG_R - t)) << 1 | y0 >= x0 + (y2 >> LOG_R - t))
            if (NW(QC(Q, i)))
                return;
        p[(y << LOG_R) + x] = O[8];
        do { B(Q); Q = QF(Q); } while (ACB(Q));
        ...
        for (i = 7; i >= 0; F[i][0] = mulsar2(x, V(i, 2)) - V(i, 2), F[i][1] = mulsar2(y, V(i, 2)) - V(i, 2), i--);
...
#else
#define DIR(i)  ((V##i(1) >= mulsar2(y + (1 << t >> 1), V##i(2)) - V##i(2)) << 1 | V##i(0) >= mulsar2((x + (1 << t >> 1)), V##i(2)) - V##i(2))
...
void M3D2D(int x, int y, int t, int y0, int y1, int y2, int z, int O[])
{
    ...
    if (O_) {
        if (y0 < -y2 || y1 < -y2 || y0 >= y2 || y1 >= y2)
            return;
        for (x = y = 0, j = k = -y2, t = LOG_R, z = 3, i = (y1 >= k + (y2 >> LOG_R - t)) << 1 | y0 >= j + (y2 >> LOG_R - t); t; j += i << 31 >> 31 & (y2 >> LOG_R - t), k += i << 30 >> 31 & (y2 >> LOG_R - t), --t, x += (i & 0x1) << t, y += i >> 1 << t, z = QC(z, i), i = (y1 >= k + (y2 >> LOG_R - t)) << 1 | y0 >= j + (y2 >> LOG_R - t))
            if (NW(QC(z, i)))
                return;
        p[(y << LOG_R) + x] = O[8];
        do { B(z); z = QF(z); } while (ACB(z));
    } else {
        if (i = CODE()) {
...
void M3D2D(int x, int y, int t, int y0, int y1, int y2, int z, int O[])
{
    ...
    if (O_) {
        ...
        if (y0 < -y2 || y1 < -y2 || y0 >= y2 || y1 >= y2)
            return;
        for (x = y = 0, t = LOG_R, z = 3, L0 = U0 = -y2, R0 = D0 = y2, i = DIR(0); t; --t, x += (i & 0x1) << t, y += i >> 1 << t, L0 = L4(0, i), U0 = U4(0, i), R0 = R4(0, i), D0 = D4(0, i), z = QC(z, i), i = DIR(0))
            if (NW(QC(z, i)))
                return;
        p[(y << LOG_R) + x] = O[8];
        do { B(z); z = QF(z); } while (ACB(z));
        ...
        if (i = CODE()) {
            ...
            D2D3M(x, y, t, y0, y1, y2, z, O, 0, 0, LOG_R, 3, -V0(2), -V0(2), V0(2), V0(2), -V1(2), -V1(2), V1(2), V1(2), -V2(2), -V2(2), V2(2), V2(2), -V3(2), -V3(2), V3(2), V3(2), -V4(2), -V4(2), V4(2), V4(2), -V5(2), -V5(2), V5(2), V5(2), -V6(2), -V6(2), V6(2), V6(2), -V7(2), -V7(2), V7(2), V7(2));
    }
}
#else
...
void D2D3M(int x0, int x1, int x2, int y0, int y1, int y2, int z, int O[], int x, int y, int t, unsigned Q, int L0, int U0, int L1, int U1, int L2, int U2, int L3, int U3, int L4, int U4, int L5, int U5, int L6, int U6, int L7, int U7)
{
    ...
    if (O_) {
        for (i = DIR(0); t; L0 += i << 31 >> 31 & V0(2) >> LOG_R - t, U0 += i << 30 >> 31 & V0(2) >> LOG_R - t, --t, x += (i & 0x1) << t, y += i >> 1 << t, Q = QC(Q, i), i = DIR(0))
            if (NW(QC(Q, i)))
                return;
...
#define x2  t
    if (O_) {
        if (y0 < -y2 || y1 < -y2 || y0 >= y2 || y1 >= y2)
            return;
        for (x = y = 0, t = LOG_R, z = 3, j = k = -y2, i = (V0(1) >= k + (V0(2) >> LOG_R - t)) << 1 | V0(0) >= j + (V0(2) >> LOG_R - t); t; j += i << 31 >> 31 & V0(2) >> LOG_R - t, k += i << 30 >> 31 & V0(2) >> LOG_R - t, --t, x += (i & 0x1) << t, y += i >> 1 << t, z = QC(z, i), i = (V0(1) >= k + (V0(2) >> LOG_R - t)) << 1 | V0(0) >= j + (V0(2) >> LOG_R - t))
            if (NW(QC(z, i)))
                return;
        p[(y << LOG_R) + x] = O[8] << 8 & 0xff0000 | O[8] >> 8 & 0xff00 | O[8] & 0xff;
        do { B(z); z = QF(z); } while (ACB(z));
