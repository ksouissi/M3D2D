...
#define D(x, y, z, t)   (x >> t & 0x1 | y >> t << 1 & 0x2 | z >> t << 2 & 0x4)
...
void merge(int t[][4], int n, int octree[], int l, int r)
{
    ...
    for (i = 0; i < n; i++) {
        k = octree;
        for (j = l - 1; j >= r; j--) {
            if (!k[D(t[i][0], t[i][1], t[i][2], j)]) {
                z8 += 9;
                k[D(t[i][0], t[i][1], t[i][2], j)] = z8;
            }
            k = k[D(t[i][0], t[i][1], t[i][2], j)];
        }
        k[8] = t[i][3];
    }
}
...
enum { A, Q, Y, F = Y + 3, S1ES = F + 4 * 4, X = S1ES + 4 * 4, O = X + 3, Z, S0ES };

#define NW(x)   (quadtrees[x >> 5] & 0x3 << (x & 0x1f))
#define NB(x)   ~quadtrees[x >> 5] & 0x3 << (x & 0x1f)

/* #define PURE */

#ifdef PURE
#define QS  ((2 * ((1 << (LOG_R + 2 + 1 << 1)) - 1) / 3 + 6 + 31) / 32)
#define QR()    quadtrees[0] = 0x3f10, quadtrees[1] = 0xfc00, ZeroMemory(quadtrees + 2, QS - 2 << 2)
#else
#define QS  ((2 * ((1 << (LOG_R + 1 << 1)) - 1) / 3 + 6 + 31) / 32)
#define QR()    quadtrees[0] = 0x10, ZeroMemory(quadtrees + 1, QS - 1 << 2)
#endif

#define B(x)    quadtrees[x >> 5] |= 0x3 << (x & 0x1f)
#define BW(x)   quadtrees[x >> 5] |= 0x1 << (x & 0x1f)
#define CNB(x)  (~quadtrees[QC(x, 0) >> 5] & 0xff << (QC(x, 0) & 0x1f))
#define ACB(x)  !CNB(x)

#define QF(x)   ((x >> 3) + 2 << 1)
#define QC(x, y)    ((x - 4 << 2) + (y << 1))
...
void RC(int L, int U, int R, int D, int x, int y, int z, unsigned Q, int c)
{
    ...
    if (x < L || y < U || x + (1 << z) > R || y + (1 << z) > D) {
        for (i = 3; i >= 0; i--)
            if (NB(QC(Q, i)) && x + ((i & 0x1) << z - 1) < R && y + (i >> 1 << z - 1) < D && x + (1 << z - (~i & 0x1)) > L && y + (1 << z - (~i >> 1 & 0x1)) > U)
                RC(L, U, R, D, x + ((i & 0x1) << z - 1), y + (i >> 1 << z - 1), z - 1, QC(Q, i), c);
        quadtrees[Q >> 5] |= (ACB(Q) << 1 | 0x1) << (Q & 0x1f);
    } else
        for (
#ifdef PURE
            x -= 1 << LOG_R << 1, y -= 1 << LOG_R << 1, 
#endif
                                                        i = y, B(Q); i < y + (1 << z); i++)
            for (j = x; j < x + (1 << z); j++)
...
void QD(int L0, int U0, int R0, int D0, int L1, int U1, int R1, int D1, int L2, int U2, int R2, int D2, int L3, int U3, int R3, int D3, int L4, int U4, int R4, int D4, int L5, int U5, int R5, int D5, int L6, int U6, int R6, int D6, int L7, int U7, int R7, int D7, int y0, int y1, int y2, int z, int z0, int z1, int t, unsigned Q, int c)
{
    int i, j, k, G[9][4][2];
...
#define V(x, y, z)  (x[Y + z] + ((y << 31 >> 31 & B[0][z]) + (y << 30 >> 31 & B[1][z]) + (y << 29 >> 31 & B[2][z]) << (x[A] & 0xff)))
#define V0(x, y)    V(E0, x, y)
#define V1(x, y)    (E1[Y + y] + ((x << 31 >> 31 & B[E1[A] >> 8 & 0xf][y]) + (x << 30 >> 31 & B[E1[A] >> 12][y]) << (E1[A] & 0xff)))

#define F(x, y, z, t)   x[F + y + ((z) << 2) + t]
#define F0(x, y)    F(E0, 0, x, y)
#define F1(x, y)    F(E1, 0, x, y)

#define PUSH_S0()   \
do {    \
    for (i = 7, j = ((V[2] & (256 << OH) - 1) >= E0[X + 2] + (128 << (E0[A] & 0xff))) << 2 | ((V[1] & (256 << OH) - 1) >= E0[X + 1] + (128 << (E0[A] & 0xff))) << 1 | (V[0] & (256 << OH) - 1) >= E0[X] + (128 << (E0[A] & 0xff)), k = j ^ i; i >= 0; i--, k = j ^ i)   \
        if (((int *)E0[O])[k]) {    \
            int i;  \
            S0[S0i + A] = E0[A] - 1 | (j ^ k) << 12;    \
            S0[S0i + Q] = E0[Q];    \
            for (i = 2; i >= 0; i--) {  \
                S0[S0i + X + i] = E0[X + i] + ((k >> i & 0x1) << 8 << (S0[S0i + A] & 0xff));    \
                S0[S0i + Y + i] = E0[Y + i] + ((k << 31 >> 31 & B[0][i]) + (k << 30 >> 31 & B[1][i]) + (k << 29 >> 31 & B[2][i]) << (S0[S0i + A] & 0xff));  \
            }   \
            for (i = 7; i >= 0; i--) {  \
                int j;  \
                for (j = 3; j >= 0; j--)    \
                    F(S0, S0i, i, j) = M(F0(k, j), F0(i, j));   \
            }   \
            S0[S0i + O] = ((int *)E0[O])[k];    \
            S0[S0i + Z] = E0[Z];    \
            S0i += S0ES;    \
...
#define POP_S0()    for (i = S0ES - 1, S0i -= S0ES; i >= 0; i--) E0[i] = S0[S0i + i]

#define PUSH_FF_S1()    \
do {    \
    for (i = 2, S1i = 0; i >= 0; i--)   \
        if (E0[A] & 0x1000 << i) {  \
            S1[S1i + A] = E0[A] & 0xff | 0x10022100 >> (i << 3) & 0xff00;   \
            S1[S1i + Q] = E0[Q];    \
            for (j = 2, k = (V[i] & (256 << OH) - 1) >= E0[X + i]; j >= 0; j--) \
                S1[S1i + Y + j] = E0[Y + j] + (-k & B[i][j] << (E0[A] & 0xff)); \
            for (j = 3, k <<= i; j >= 0; j--) { \
                int i;  \
                for (i = 3; i >= 0; i--)    \
                    F(S1, S1i, j, i) = F0(k | (j & 0x1) << (S1[S1i + A] >> 8 & 0xf) | j >> 1 << (S1[S1i + A] >> 12), i);    \
            }   \
            S1i += S1ES;    \
        }   \
} while (0)
...
do {    \
    for (i = 3; i >= 0; i--) {  \
        ...
        S1[S1i + Q] = E1[Q];    \
        for (j = 2; j >= 0; j--)    \
            S1[S1i + Y + j] = E1[Y + j] + ((i << 31 >> 31 & B[E1[A] >> 8 & 0xf][j]) + (i << 30 >> 31 & B[E1[A] >> 12][j]) << (S1[S1i + A] & 0xff)); \
        for (j = 3; j >= 0; j--)    \
            for (k = 3; k >= 0; k--)    \
                F(S1, S1i, j, k) = M(F1(i, k), F1(j, k));   \
        S1i += S1ES;    \
    }   \
} while (0)

#define POP_S1()    for (i = S1ES - 1, S1i -= S1ES; i >= 0; i--) E1[i] = S1[S1i + i]

#define D0(x)   ((V0(x, 1) >= M(F0(x, 1), F0(x, 3))) << 1 | V0(x, 0) >= M(F0(x, 0), F0(x, 2)))
#define D1(x)   ((V1(x, 1) >= M(F1(x, 1), F1(x, 3))) << 1 | V1(x, 0) >= M(F1(x, 0), F1(x, 2)))

#define L(x, y) x[F + (y << 2)]
#define U(x, y) x[F + (y << 2) + 1]
#define R(x, y) x[F + (y << 2) + 2]
#define D(x, y) x[F + (y << 2) + 3]

#define L_4(x, y, z)    ~z << 31 >> 31 & L(x, y) | z << 31 >> 31 & M(L(x, y), R(x, y))
#define U_4(x, y, z)    ~z << 30 >> 31 & U(x, y) | z << 30 >> 31 & M(U(x, y), D(x, y))
#define R_4(x, y, z)    z << 31 >> 31 & R(x, y) | ~z << 31 >> 31 & M(L(x, y), R(x, y))
#define D_4(x, y, z)    z << 30 >> 31 & D(x, y) | ~z << 30 >> 31 & M(U(x, y), D(x, y))

#define L0_4(x, y)  L_4(E0, x, y)
#define U0_4(x, y)  U_4(E0, x, y)
#define R0_4(x, y)  R_4(E0, x, y)
#define D0_4(x, y)  D_4(E0, x, y)

#define L1_4(x, y)  L_4(E1, x, y)
#define U1_4(x, y)  U_4(E1, x, y)
#define R1_4(x, y)  R_4(E1, x, y)
#define D1_4(x, y)  D_4(E1, x, y)

#define QL  (E0[A] & 0xf00)
#define QL_1()  E0[A] -= 0x100

#define F0_4(x, y)  F0(x, 0) = L0_4(x, y), F0(x, 1) = U0_4(x, y), F0(x, 2) = R0_4(x, y), F0(x, 3) = D0_4(x, y)
#define F1_4(x, y)  F1(x, 0) = L1_4(x, y), F1(x, 1) = U1_4(x, y), F1(x, 2) = R1_4(x, y), F1(x, 3) = D1_4(x, y)

#define Q1  E0[A] & 0xf00
#define Z_4(x)  E0[Z] += (x >> 1 << 16 | x & 0x1) << (E0[A] >> 8 & 0xf)

#define ABS(x)  (x >> 31 ^ x - ((unsigned)x >> 31))
#define OQ_(x)  (256 << (E##x[A] & 0xff)) - 256 <= /* ABS(E##x[Y]) + ABS(E##x[Y + 1]) + ABS(E##x[Y + 2]) */ E##x[Y + 2] >> LOG_R - 1

#ifdef PURE
void M3D2D()
{
    int S0[(7 * OH + 1) * S0ES], S0i, E0[S0ES], S1[(2 + 3 * OH + 1) * S1ES], S1i, E1[S1ES];
    int i, j, k;
    ...
    for (i = 2; i >= 0; i--) {
        S0[X + i] = 0;
        S0[Y + i] = -((V[0] & (256 << OH) - 1) >> 6) * B[0][i] - ((V[1] & (256 << OH) - 1) >> 6) * B[1][i] - ((V[2] & (256 << OH) - 1) >> 6) * B[2][i] >> 2;
    }
    for (i = 7; i >= 0; i--) {
        F(S0, 0, i, 0) = F(S0, 0, i, 1) = -(V(S0, i, 2) << 2) - V(S0, i, 2);
        F(S0, 0, i, 2) = F(S0, 0, i, 3) = (V(S0, i, 2) << 1) + V(S0, i, 2);
    }
    S0[O] = octrees;
    S0[Z] = 0;
    S0i = S0ES;
...
L0:
            if (OQ_(0)) {
                for (QD(F0(0, 0), F0(0, 1), F0(0, 2), F0(0, 3), F0(1, 0), F0(1, 1), F0(1, 2), F0(1, 3), F0(2, 0), F0(2, 1), F0(2, 2), F0(2, 3), F0(3, 0), F0(3, 1), F0(3, 2), F0(3, 3), F0(4, 0), F0(4, 1), F0(4, 2), F0(4, 3), F0(5, 0), F0(5, 1), F0(5, 2), F0(5, 3), F0(6, 0), F0(6, 1), F0(6, 2), F0(6, 3), F0(7, 0), F0(7, 1), F0(7, 2), F0(7, 3), E0[Y], E0[Y + 1], E0[Y + 2], E0[A] & 0xff, E0[Z] & 0xffff, E0[Z] >> 16, E0[A] >> 8 & 0xf, E0[Q], ((int *)E0[O])[8] << 8 & 0xff0000 | ((int *)E0[O])[8] >> 8 & 0xff00 | ((int *)E0[O])[8] & 0xff);;) {
                    E0[Q] = QF(E0[Q]);
                    if (CNB(E0[Q]))
                        break;
                    B(E0[Q]);
                ...
                for (;;)
                    if ((i = D0(0)) ^ D0(1) || i ^ D0(2) || i ^ D0(3) || i ^ D0(4) || i ^ D0(5) || i ^ D0(6) || i ^ D0(7)) {    /* O in !Qi */
                        if (NW(E0[Q])) {
                            PUSH_FF_S1();
                            do {
L1:
...
void M3D2D()
{
    ...
    for (i = 7; i >= 0; i--) {
        F(S0, 0, i, 0) = F(S0, 0, i, 1) = -V(S0, i, 2);
        F(S0, 0, i, 2) = F(S0, 0, i, 3) = V(S0, i, 2);
    }
    S0[O] = octrees;
    S0[Z] = 0;
    S0i = S0ES;
...
L0:
                ...
                /* !!O */
                for (i = 7, j = 0; i >= 0; i--)
                    j |= ((V0(i, 1) >= F0(i, 3)) << 3 | (V0(i, 0) >= F0(i, 2)) << 2 | (V0(i, 1) < F0(i, 1)) << 1 | V0(i, 0) < F0(i, 0)) << (i << 2);
                if (j) {    /* !in */
                    if (j & j >> 4 & j >> 8 & j >> 12 & j >> 16 & j >> 20 & j >> 24 & (unsigned)j >> 28)    /* out */
                        continue;
                    /* !in & !out */
                    PUSH_S0();
                    POP_S0();
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        BitBlt(hdc, 0, 0, 1 << LOG_R, 1 << LOG_R, hdcm, 0, 0, SRCCOPY);
        EndPaint(hwnd, &ps);
        return 0;
    case WM_MOUSEMOVE:
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    bmi.bmiHeader.biHeight = -(1 << LOG_R);
    bmi.bmiHeader.biCompression = BI_RGB;

    GetWindowRect(hwnd, &rec);
    GetClientRect(hwnd, &rect);
    rec.right = rect.left - rec.left + (1 << LOG_R) + rec.right - rect.right;
    rec.bottom = rect.top - rec.top + (1 << LOG_R) + rec.bottom - rect.bottom;
