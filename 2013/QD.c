...
void RC(int L, int U, int R, int D, int c)
{
    ...
    if (L >> 5 ^ (k = R >> 5))
        for (i = U; i < D; i++) {
            W[YMXR[i] + L >> 5] &= (1 << (L & 0x1f)) - 1; W[YMXR[i] + R >> 5] &= -(1 << (R & 0x1f));
            for (j = L + 32 >> 5; j < k; j++)
                W[YMXR[i] + (j << 5) >> 5] = 0;
        }
    else
    ...
    for (; U < D; U++)
        for (i = L; i < R; i++)
            p[YMXR[U] + i] = -!p[YMXR[U] + i] & c | p[YMXR[U] + i];
}
...
void D2D3M(int L0, int U0, int R0, int D0, int L1, int U1, int R1, int D1, int L2, int U2, int R2, int D2, int L3, int U3, int R3, int D3, int L4, int U4, int R4, int D4, int L5, int U5, int R5, int D5, int L6, int U6, int R6, int D6, int L7, int U7, int R7, int D7, int L8, int U8, int R8, int D8, int x0, int x1, int x2, int y0, int y1, int y2, int z, unsigned O[])
{
    ...
    if (NR(G[8][0][0] >> Z, G[8][1][0] >> Z, G[8][2][0] >> Z, G[8][3][0] >> Z)) {
        if (G[8][2][0] - G[8][0][0] <= CN << Z || G[8][3][0] - G[8][1][0] <= CN << Z || !z    /* O_ */ /* (256 << z) - 256 <= ABS(y0) + ABS(y1) + ABS(y2) >> LOG_R - 1 */) {
            if (y2 >= 0)
...
/*        for (j = 0; j < 8; j++)
    ...
    for (;;) {
        if (PeekMessage(&msg, NULL, 0, 0, PM_REMOVE)) {
            ...
            memset(W, 0xff, XR * YR / 8);
            D2D3M(-V(0, 2), -V(0, 2) * YX, V(0, 2), V(0, 2) * YX, -V(1, 2), -V(1, 2) * YX, V(1, 2), V(1, 2) * YX, -V(2, 2), -V(2, 2) * YX, V(2, 2), V(2, 2) * YX, -V(3, 2), -V(3, 2) * YX, V(3, 2), V(3, 2) * YX, -V(4, 2), -V(4, 2) * YX, V(4, 2), V(4, 2) * YX, -V(5, 2), -V(5, 2) * YX, V(5, 2), V(5, 2) * YX, -V(6, 2), -V(6, 2) * YX, V(6, 2), V(6, 2) * YX, -V(7, 2), -V(7, 2) * YX, V(7, 2), V(7, 2) * YX, 0, 0, XR << Z, YR << Z, 0, 0, 0, V(0, 0), V(0, 1), V(0, 2), OH, octrees);
            BitBlt(hdc, 0, 0, XR, YR, hdcm, 0, 0, SRCCOPY);
